...
export function OrderItemsList({ items }: OrderItemsListProps) {
  const computedItemTotal = (item: OrderItem) => {
    const basePrice = parseFloat(item.price.toString())
    const modifierUnitTotal = item.modifiers?.reduce(
      (sum, mod) => sum + parseFloat(mod.price_modifier.toString()),
      0
    ) ?? 0
    return {
      effectiveUnitPrice: basePrice + modifierUnitTotal,
      modifierUnitTotal,
      total: (basePrice + modifierUnitTotal) * item.quantity,
    }
  }
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-3">
        {items.map((item) => {
          const { effectiveUnitPrice, modifierUnitTotal, total } = computedItemTotal(item)
              ...
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Quantity: {item.quantity} × ${effectiveUnitPrice.toFixed(2)}
                  </p>
                  {modifierUnitTotal > 0 && (
                    <p className="text-xs text-gray-500">
                      (base ${parseFloat(item.price.toString()).toFixed(2)} + ${modifierUnitTotal.toFixed(2)} modifiers)
                    </p>
                  )}
                </div>
